...
private:
    ...
    // total number of particles is num_particles_width_ * num_particles_height_
    int constraint_iterations_;

    int rigidness_;
    double time_step_;

    std::vector <Particle> particles_; // all particles that are part of this cloth

    double smoothThreshold_;
...
public:
    ...
    std::vector<double> heightvals_; // height values
    int num_particles_width_;   // number of particles in width direction
    int num_particles_height_;  // number of particles in height direction
...
public:
    ...
    int getSize() {
        return num_particles_width_ * num_particles_height_;
    }

    std::size_t get1DIndex(int x, int y) {
        return y * num_particles_width_ + x;
    }
...
public:
    ...
    void outputMesh(vtkSmartPointer <vtkPolyData>& mesh); // Zexin
};
