...
#define ROTATE(a, i, j, k, l) { Scalar g = a[i][j]; h = a[k][l]; a[i][j] = g-s*(h+g*tau); a[k][l] = h+s*(g-h*tau); }
    ...
    public:
        ...
        **/
        static bool
        ComputeEigenValuesAndVectors2(const SquareMatrix& matrix, SquareMatrix& eigenVectors, EigenValues& eigenValues,
                ...
                for (unsigned ip = 0; ip < n - 1; ip++) {
                    for (unsigned iq = ip + 1; iq < n; iq++) {
                        ...
                        if (i > 4
                            ...
                            {
                                for (unsigned j = 0; j + 1 <= ip; j++) ROTATE(matrix.m_values, j, ip, j, iq)
                            }
                            //Case of rotations p < j < q
                            {
                                for (unsigned j = ip + 1; j + 1 <= iq; j++) ROTATE(matrix.m_values, ip, j, j, iq)
                            }
                            //Case of rotations q < j <= n
                            {
                                for (unsigned j = iq + 1; j < n; j++) ROTATE(matrix.m_values, ip, j, iq, j)
                            }
                            //Last case
                            {
                                for (unsigned j = 0; j < n; j++) ROTATE(eigenVectors.m_values, j, ip, j, iq)
                            }
                        }
                    }
                }
        ...
        **/
        ...
        \return success
        **/
        static bool GetMaxEigenValueAndVector(const SquareMatrix& eigenVectors, const EigenValues& eigenValues,
                                              Scalar& maxEigenValue, Scalar maxEigenVector[]) {
            if (!eigenVectors.isValid()
                || eigenVectors.size() < 2
                || eigenVectors.size() != eigenValues.size()) {
                assert(false);
        ...
        \return success
        **/
        static bool GetMinEigenValueAndVector(const SquareMatrix& eigenVectors, const EigenValues& eigenValues,
                                              Scalar& minEigenValue, Scalar minEigenVector[]) {
            if (!eigenVectors.isValid()
                || eigenVectors.size() < 2
                || eigenVectors.size() != eigenValues.size()) {
                assert(false);
