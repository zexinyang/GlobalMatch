...
public:
    ...
    **/
    ...
    inline Vector3Tpl cross(const Vector3Tpl& v) const {
        return Vector3Tpl((y * v.z) - (z * v.y), (z * v.x) - (x * v.z), (x * v.y) - (y * v.x));
    }
    ...
    inline void normalize() {
        Type n = norm();
        if (n > std::numeric_limits<Type>::epsilon()) *this /= n;
    }
    ...
    static inline double vdotd(const Type p[], const Type q[]) {
        return (static_cast<double>(p[0]) * q[0]) + (static_cast<double>(p[1]) * q[1]) +
    ...
    static inline double vnorm2d(const Type p[]) {
        return (static_cast<double>(p[0]) * p[0]) + (static_cast<double>(p[1]) * p[1]) +
    ...
    static inline double vdistance2d(const Type p[], const Type q[]) {
        return ((static_cast<double>(p[0]) - q[0]) * (static_cast<double>(p[0]) - q[0])) +
               ((static_cast<double>(p[1]) - q[1]) * (static_cast<double>(p[1]) - q[1])) +
               ((static_cast<double>(p[2]) - q[2]) * (static_cast<double>(p[2]) - q[2]));
    }
